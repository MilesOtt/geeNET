...
gee.net<-function(form, link, dist.matrix, dataset, SE="sandwich", weights==NULL){
  ...
  a<-quasi.score(initialvalues, dist.matrix, x.matrix, link, y, W) #get betas, rho
  ...
  return(list(summary, rho.hat, SE))
...
gee.net(formula, link.here, distance.matrix, data=PigDiet,  SE="sandwich")
...
gee.net(formula, link, dist.matrix, dataset=ohio.1,  SE="sandwich")
...
link="logit"
gee.net(Marijuana.ever~Male+frat+heavy.drinks, link, dist.matrix, dataset=ur.data,  SE="model.based")
...
link="identity"
gee.net(heavy.drinks~Male+frat+drink.days, link, dist.matrix, dataset=ur.data, SE="model.based")
